...
int main(int argc, char* argv[]) {
	...
	if (pid2 == -1) {
	    ...
	    // Writing answer to pipe.
            size = write(ans_fd[1], &ans, sizeof(int));
	...
	// Waiting till 2-st process is done reading data from pipes, computing result and transferring it back to 1-st process.
	...
	// Opening and writing answer to output file.
        ...
        if(close(ans_fd[0]) < 0) {
            printf("First process can\'t close reading side of pipe\n"); 
            exit(-1);
        }
    ...
    // Waiting till 1-st process is done reading string and transferring data and then writing answer to file.
